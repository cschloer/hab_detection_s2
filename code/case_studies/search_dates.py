...
from get_cyan import (
    search_sentinel2,
    download_cyan_geotiff,
    crop_cyan_to_bbox,
    get_cyan_url,
)
...
def search_for_suitable_date():
    ...
    while dates_checked < MAX_DAYS_TO_SEARCH:
        try:
            ...
            try:
                sen2_product = search_sentinel2(
                    BOUNDING_BOX, current_date, date_range_days=0
                )
            ...
            if os.path.exists(cached_cyan_path):
                ...
                try:
                    download_cyan_geotiff(
                        cached_cyan_path, current_date, CYAN_REGION_ID
                    )
                    original_cyan_path = cached_cyan_path
            ...
            try:
                ...
                has_meaningful, meaningful_ratio, stats = check_cyan_meaningful_data(
                    cyan_array
                )
                ...
                if has_meaningful:
                    ...
                    print(
                        f"  ✗ Insufficient meaningful CyaN data ({meaningful_ratio:.1%})"
                    )
...
def main():
    ...
    if result_date:
        ...
        print(
            f"Unique CyaN values in cropped data: {unique_values[:10]}{'...' if len(unique_values) > 10 else ''}"
        )
